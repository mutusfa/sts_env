...
def test_small_slimes_reproducible():
    obs1 = encounters.small_slimes(seed=42, character=PlayerState()).observe()
    obs2 = encounters.small_slimes(seed=42, character=PlayerState()).observe()
...
def test_two_louses_names_are_louses():
    for seed in range(20):
        obs = encounters.two_louses(seed=seed, character=PlayerState()).observe()
...
def test_two_louses_reproducible():
    obs1 = encounters.two_louses(seed=7, character=PlayerState()).observe()
    obs2 = encounters.two_louses(seed=7, character=PlayerState()).observe()
...
def test_gremlin_gang_all_are_gremlins():
    for seed in range(10):
        obs = encounters.gremlin_gang(seed=seed, character=PlayerState()).observe()
        for e in obs.enemies:
...
def test_gremlin_gang_reproducible():
    obs1 = encounters.gremlin_gang(seed=99, character=PlayerState()).observe()
    obs2 = encounters.gremlin_gang(seed=99, character=PlayerState()).observe()
    assert [e.name for e in obs1.enemies] == [e.name for e in obs2.enemies]
    assert obs1.hand == obs2.hand
...
def test_spike_slime_m_reproducible():
    obs1 = encounters.spike_slime_m(seed=42, character=PlayerState()).observe()
    obs2 = encounters.spike_slime_m(seed=42, character=PlayerState()).observe()
    assert obs1.enemies[0].hp == obs2.enemies[0].hp
    assert obs1.hand == obs2.hand
...
def test_acid_slime_l_reproducible():
    obs1 = encounters.acid_slime_l(seed=7, character=PlayerState()).observe()
    obs2 = encounters.acid_slime_l(seed=7, character=PlayerState()).observe()
    assert obs1.enemies[0].hp == obs2.enemies[0].hp
    assert obs1.hand == obs2.hand
...
def test_spike_slime_l_reproducible():
    obs1 = encounters.spike_slime_l(seed=11, character=PlayerState()).observe()
    obs2 = encounters.spike_slime_l(seed=11, character=PlayerState()).observe()
    assert obs1.enemies[0].hp == obs2.enemies[0].hp
    assert obs1.hand == obs2.hand
...
def test_large_slime_varies_across_seeds():
    seen: set[str] = set()
    for seed in range(30):
        obs = encounters.large_slime(seed=seed, character=PlayerState()).observe()
...
def test_small_slimes_composition_is_small_plus_medium():
    ...
    for seed in range(20):
        obs = encounters.small_slimes(seed=seed, character=PlayerState()).observe()
...
def test_blue_slaver_factory():
    obs = encounters.blue_slaver(seed=0, character=PlayerState()).observe()
    assert len(obs.enemies) == 1
...
def test_blue_slaver_hp_in_range():
    for seed in range(10):
        obs = encounters.blue_slaver(seed=seed, character=PlayerState()).observe()
...
def test_blue_slaver_reproducible():
    obs1 = encounters.blue_slaver(seed=42, character=PlayerState()).observe()
    obs2 = encounters.blue_slaver(seed=42, character=PlayerState()).observe()
    assert obs1.enemies[0].hp == obs2.enemies[0].hp
...
def test_red_slaver_factory():
    obs = encounters.red_slaver(seed=0, character=PlayerState()).observe()
    assert len(obs.enemies) == 1
...
def test_red_slaver_hp_in_range():
    for seed in range(10):
        obs = encounters.red_slaver(seed=seed, character=PlayerState()).observe()
        assert 46 <= obs.enemies[0].hp <= 50
...
def test_looter_factory():
    obs = encounters.looter(seed=0, character=PlayerState()).observe()
    assert len(obs.enemies) == 1
...
def test_looter_reproducible():
    obs1 = encounters.looter(seed=7, character=PlayerState()).observe()
    obs2 = encounters.looter(seed=7, character=PlayerState()).observe()
    assert obs1.enemies[0].hp == obs2.enemies[0].hp
...
def test_three_louse_has_three_enemies():
    obs = encounters.three_louse(seed=0, character=PlayerState()).observe()
    assert len(obs.enemies) == 3


def test_three_louse_all_are_louses():
    for seed in range(10):
        obs = encounters.three_louse(seed=seed, character=PlayerState()).observe()
        for e in obs.enemies:
            assert e.name in {"RedLouse", "GreenLouse"}, (
...
def test_three_louse_reproducible():
    obs1 = encounters.three_louse(seed=5, character=PlayerState()).observe()
    obs2 = encounters.three_louse(seed=5, character=PlayerState()).observe()
    assert [e.name for e in obs1.enemies] == [e.name for e in obs2.enemies]
...
def test_two_fungi_beasts_has_two_enemies():
    obs = encounters.two_fungi_beasts(seed=0, character=PlayerState()).observe()
    assert len(obs.enemies) == 2
...
def test_lots_of_slimes_has_five_enemies():
    obs = encounters.lots_of_slimes(seed=0, character=PlayerState()).observe()
...
def test_lots_of_slimes_correct_pool():
    ...
    from collections import Counter
    for seed in range(20):
        obs = encounters.lots_of_slimes(seed=seed, character=PlayerState()).observe()
        counts = Counter(e.name for e in obs.enemies)
...
def test_lots_of_slimes_reproducible():
    obs1 = encounters.lots_of_slimes(seed=99, character=PlayerState()).observe()
    obs2 = encounters.lots_of_slimes(seed=99, character=PlayerState()).observe()
    assert [e.name for e in obs1.enemies] == [e.name for e in obs2.enemies]
...
def test_exordium_thugs_has_two_enemies():
    obs = encounters.exordium_thugs(seed=0, character=PlayerState()).observe()
    assert len(obs.enemies) == 2


def test_exordium_thugs_composition():
    for seed in range(20):
        obs = encounters.exordium_thugs(seed=seed, character=PlayerState()).observe()
        names = [e.name for e in obs.enemies]
...
def test_exordium_thugs_reproducible():
    obs1 = encounters.exordium_thugs(seed=13, character=PlayerState()).observe()
    obs2 = encounters.exordium_thugs(seed=13, character=PlayerState()).observe()
    assert [e.name for e in obs1.enemies] == [e.name for e in obs2.enemies]
...
def test_exordium_wildlife_has_two_enemies():
    obs = encounters.exordium_wildlife(seed=0, character=PlayerState()).observe()
    assert len(obs.enemies) == 2


def test_exordium_wildlife_composition():
    for seed in range(20):
        obs = encounters.exordium_wildlife(seed=seed, character=PlayerState()).observe()
        names = [e.name for e in obs.enemies]
...
def test_exordium_wildlife_reproducible():
    obs1 = encounters.exordium_wildlife(seed=17, character=PlayerState()).observe()
    obs2 = encounters.exordium_wildlife(seed=17, character=PlayerState()).observe()
    assert [e.name for e in obs1.enemies] == [e.name for e in obs2.enemies]
...
def test_act1_weak_encounter_varies_across_seeds():
    ...
    for seed in range(40):
        obs = encounters.act1_weak_encounter(seed=seed, character=PlayerState()).observe()
        seen.add(obs.enemies[0].name)
...
def test_act1_weak_encounter_reproducible():
    obs1 = encounters.act1_weak_encounter(seed=42, character=PlayerState()).observe()
    obs2 = encounters.act1_weak_encounter(seed=42, character=PlayerState()).observe()
    assert [e.name for e in obs1.enemies] == [e.name for e in obs2.enemies]
...
def test_act1_strong_encounter_varies_across_seeds():
    ...
    for seed in range(60):
        obs = encounters.act1_strong_encounter(seed=seed, character=PlayerState()).observe()
...
def test_act1_strong_encounter_reproducible():
    obs1 = encounters.act1_strong_encounter(seed=99, character=PlayerState()).observe()
    obs2 = encounters.act1_strong_encounter(seed=99, character=PlayerState()).observe()
    assert [e.name for e in obs1.enemies] == [e.name for e in obs2.enemies]
